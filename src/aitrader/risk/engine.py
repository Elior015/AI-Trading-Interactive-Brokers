...
class RiskEngine:
    ...
    def evaluate_close(
        ...
        """Minimal gate for model-initiated CLOSE proposals.

        Exits are exempt from sizing/entry limits by design (`evaluate()`) —
        closing risk isn't new risk. But `close_position()` still reaches the
        broker, so it cannot go through with no gate at all: the kill switch
        must actually stop everything, including exits, or an operator who
        trips it over a data anomaly is trusting the model not to act on the
        same bad data while it decides what to close. Session phase, a real
        quote, and a same-symbol working-order interlock are checked for the
        same reason `evaluate()` checks them for entries.
        """
        if self.kill_switch.is_tripped:
            return RiskVerdict.reject(
                RejectReason.KILL_SWITCH, self.kill_switch.reason or "kill switch active", symbol
            )
        if not phase.is_tradeable:
            return RiskVerdict.reject(
                RejectReason.MARKET_CLOSED, f"session phase is {phase.value}", symbol
            )
        if symbol in working_symbols:
            return RiskVerdict.reject(
                RejectReason.DUPLICATE_ORDER,
                "an order for this symbol is already working",
                symbol,
            )
        if quote is None:
            return RiskVerdict.reject(RejectReason.NO_MARKET_DATA, "no quote available", symbol)
        if quote.halted:
            return RiskVerdict.reject(RejectReason.HALTED, "contract is halted", symbol)
        if not quote.is_usable:
            return RiskVerdict.reject(
                RejectReason.NO_MARKET_DATA, "quote has no usable price", symbol
            )
        qage = quote.age_seconds()
        if qage > self.cfg.max_quote_age_seconds:
            return RiskVerdict.reject(
                RejectReason.STALE_MARKET_DATA,
                f"quote is {qage:.0f}s old (max {self.cfg.max_quote_age_seconds:.0f}s)",
                symbol,
            )
        return RiskVerdict.ok(symbol)
